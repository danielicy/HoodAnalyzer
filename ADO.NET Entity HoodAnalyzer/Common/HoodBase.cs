...
namespace EmployeeTracker.Common
{
    public class HoodBase
    {
        ...
        public AnalysisParams[] Analysis = new AnalysisParams[1];
        ...
        public struct AnalysisParams
        {
            ...
            string InputFileNameGeneric;
            string InputFieldName;
            enumAnalysisType Type;
            enumSpatialReletionship SpatialReletionships;
            double Radius;
            ClusterParam[] CP;
        }
...
//'    Set FeatClass = OpenShapefile(OutputFilePath, "tmpFishnet")
...
//Public Function CustomToolsCreateFishnet(Path As String, Name As String, _
//x As Double, y As Double, Rows As Long, Cols As Long, _
//Width As Double, Height As Double, FishnetType As enumFishnetType, _
//SpatialReference As ISpatialReference, AddSpatialIndex As Boolean, AddAsLayer As Boolean) As Boolean
...
//Dim blnSpatialIndex As Boolean, blnGeomIsPoly As Boolean
...
//    If FishnetType = PolygonsFishnet Then
//      blnGeomIsPoly = True
//    Else
//      blnGeomIsPoly = False
//    End If
...
//    If blnGeomIsPoly Then
//        pFieldsEdit.AddField CreateField("Col_Row", _
//          esriFieldTypeString, 20)
//        pFieldsEdit.AddField CreateField("Col", _
//          esriFieldTypeInteger)
//        pFieldsEdit.AddField CreateField("Row", _
//          esriFieldTypeInteger)
//        pFieldsEdit.AddField CreateField("MinX", _
//          esriFieldTypeDouble)
//        pFieldsEdit.AddField CreateField("MinY", _
//          esriFieldTypeDouble)
//        pFieldsEdit.AddField CreateField("MaxX", _
//          esriFieldTypeDouble)
//        pFieldsEdit.AddField CreateField("MaxY", _
//          esriFieldTypeDouble)
//        pFieldsEdit.AddField CreateField("CentroidX", _
//          esriFieldTypeDouble)
//        pFieldsEdit.AddField CreateField("CentroidY", _
//          esriFieldTypeDouble)
//    End If
...
//    Set pOutFeatClass = CreateShapefile(Path, Name, pFields)


//    ' the output shapefile can be either polygon or polyline.
//    Dim pPointColl As IPointCollection
//    Dim pPnt1 As IPoint, pPnt2 As IPoint
//    Dim pPnt3 As IPoint, pPnt4 As IPoint
//    Set pPnt1 = New Point
//    Set pPnt2 = New Point
//    Set pPnt3 = New Point
//    Set pPnt4 = New Point
...
//    Dim tmpFeat As IFeature

//    If blnGeomIsPoly = True Then ' if output shapefile is polygon.
...
//        Dim pFirstPolygon As IPolygon
//        Set pFirstPolygon = pPointColl
//        pFirstPolygon.Close
//        Set pClone = pFirstPolygon


//        ' write the polygons out to the shapefile.
//        On Error GoTo AfterFeatCursorOpened
//        Set pInsertFeatureCursor = pOutFeatClass.Insert(True)
//        Set pInsertFeatureBuffer = pOutFeatClass.CreateFeatureBuffer
//        Dim lngIndexPos As Long
//        lngIndexPos = pOutFeatClass.FindField("Col_Row")
//        lngColIndexPos = pOutFeatClass.FindField("Col")
//        lngRowIndexPos = pOutFeatClass.FindField("Row")
//        lngMinXIndexPos = pOutFeatClass.FindField("MinX")
//        lngMinYIndexPos = pOutFeatClass.FindField("MinY")
//        lngMaxXIndexPos = pOutFeatClass.FindField("MaxX")
//        lngMaxYIndexPos = pOutFeatClass.FindField("MaxY")
//        lngCentroidXIndexPos = pOutFeatClass.FindField("CentroidX")
//        lngCentroidYIndexPos = pOutFeatClass.FindField("CentroidY")
...
//            Set pPolygon = pClone.Clone
//            Set pTrans2D = pPolygon
//            pTrans2D.Move dblDeltaX, dblDeltaY
//            Set pInsertFeatureBuffer.Shape = pPolygon
//            pInsertFeatureBuffer.Value(lngIndexPos) = Format(I, "0000") & ":" & Format(J, "0000")
//            pInsertFeatureBuffer.Value(lngColIndexPos) = I
//            pInsertFeatureBuffer.Value(lngRowIndexPos) = J
//            pInsertFeatureBuffer.Value(lngMinXIndexPos) = pInsertFeatureBuffer.Shape.Envelope.XMin
//            pInsertFeatureBuffer.Value(lngMinYIndexPos) = pInsertFeatureBuffer.Shape.Envelope.YMin
//            pInsertFeatureBuffer.Value(lngMaxXIndexPos) = pInsertFeatureBuffer.Shape.Envelope.XMax
//            pInsertFeatureBuffer.Value(lngMaxYIndexPos) = pInsertFeatureBuffer.Shape.Envelope.XMax

//            Set tmpFeat = pInsertFeatureBuffer
//            pInsertFeatureBuffer.Value(lngCentroidXIndexPos) = GetFeatureCentroid(tmpFeat).x
//            pInsertFeatureBuffer.Value(lngCentroidYIndexPos) = GetFeatureCentroid(tmpFeat).y
//            Set tmpFeat = Nothing
...
//        Dim pHorzPolyline As IPolyline, pVertPolyline As IPolyline
...
//        pPointColl.AddPoint pPnt3
//        pPointColl.AddPoint pPnt4
//        Set pVertPolyline = pPointColl
//        ' write the polylines out to the shapefile.
//        On Error GoTo AfterFeatCursorOpened
//        Set pInsertFeatureCursor = pOutFeatClass.Insert(True)
//        Set pInsertFeatureBuffer = pOutFeatClass.CreateFeatureBuffer
//        Dim pPolyline As IPolyline
//        Set pClone = pVertPolyline
//        For I = 0 To lngCols
//          dblDeltaX = (I * dblWidth)
//          Set pPolyline = pClone.Clone
//          Set pTrans2D = pPolyline
//          pTrans2D.Move dblDeltaX, 0
//          Set pInsertFeatureBuffer.Shape = pPolyline
//          pInsertFeatureCursor.InsertFeature pInsertFeatureBuffer
//          lngCurrentFeatureCount = lngCurrentFeatureCount + 1
//          DoEvents
//        Next
//        Set pClone = pHorzPolyline
//        For I = 0 To lngRows
//          dblDeltaY = (I * dblHeight)
//          Set pPolyline = pClone.Clone
//          Set pTrans2D = pPolyline
...
//      If Not CreateSpatialIndex(pOutFeatClass) Then GoTo CantMakeSpatialIndex
//    End If
...
//Public Function NeighborhoodStatisticsEx_DATA_IS_ARRAY_NO_INVERSE(FishnetFeatClass As IFeatureClass, _
//Analysis As AnalysisParams, CellSize As Double, _
//FishnetMinX As Double, FishnetMinY As Double, Rows As Long, Cols As Long, _
//OutputFilePath As String, OutputFileName As String, OutputFileNameGeneric As String, _
//OutputDatasetType As String, SpatialReference As ISpatialReference, _
//AddSpatialIndex As Boolean, NoDataValue As Variant, AddAsLayer As Boolean) As Boolean
...
//        Case enumAnalysisType.AnalysisTypeMax
//            pOutputFeatClass.AddField CreateField( _
//            "Max", esriFieldTypeDouble)
//        Case enumAnalysisType.AnalysisTypeMin
//            pOutputFeatClass.AddField CreateField( _
//            "Min", esriFieldTypeDouble)
//        Case enumAnalysisType.AnalysisTypeStDev
//            pOutputFeatClass.AddField CreateField( _
//            "StDev", esriFieldTypeDouble)
//        Case enumAnalysisType.AnalysisTypeSum
//            pOutputFeatClass.AddField CreateField( _
//            "Sum", esriFieldTypeDouble)
//        Case enumAnalysisType.AnalysisTypeCount
...
//    If Analysis.Type = AnalysisTypeStDev Then
//        Dim NeightborhoodData() As Variant
//    End If
...
//        If FetchUserInterrupts = UserInterruptTypeStop Then Exit Function
//        If Analysis.Type = AnalysisTypeStDev Then
//            ReDim NeightborhoodData(0) As Variant
//        End If
//        s = 0: C = 0: cc = 0: mx = 0: mn = 0
//        If Analysis.Radius = 0 Then
//            MinRowIndex = 1
//            MaxRowIndex = Rows
//            MinColIndex = 1
...
//                                Case enumAnalysisType.AnalysisTypeMax
//                                    If C = 1 Then
//                                        mx = InputArr(InputIndex, 5)
//                                    Else
//                                        If InputArr(InputIndex, 5) > mx Then
//                                            mx = InputArr(InputIndex, 5)
//                                        End If
//                                    End If
//                                Case enumAnalysisType.AnalysisTypeMin
//                                    If C = 1 Then
//                                        mn = InputArr(InputIndex, 5)
//                                    Else
//                                        If InputArr(InputIndex, 5) < mn Then
//                                            mn = InputArr(InputIndex, 5)
//                                        End If
//                                    End If
//                                Case enumAnalysisType.AnalysisTypeSum
...
//                                Case enumAnalysisType.AnalysisTypeStDev
//                                    s = s + InputArr(InputIndex, 5)
//                                    If UBound(NeightborhoodData) = 0 Then
//                                        ReDim NeightborhoodData(1 To 1) As Variant
//                                    Else
//                                        ReDim Preserve NeightborhoodData(1 To _
//                                        UBound(NeightborhoodData) + 1) As Variant
//                                    End If
//                                    NeightborhoodData(UBound(NeightborhoodData)) _
//                                    = InputArr(InputIndex, 5)
//                                Case enumAnalysisType.AnalysisTypeClusterAnalysis
//                                    If IsWithinCluster(Analysis, _
//                                    InputArr(InputIndex, 5)) Then cc = cc + 1
//                            End Select
...
//                        Case enumAnalysisType.AnalysisTypeMax
//                            pOutputFeat.Value( _
//                            pOutputFeat.Fields.FindField("Max")) = mx
//                        Case enumAnalysisType.AnalysisTypeMin
//                            pOutputFeat.Value( _
//                            pOutputFeat.Fields.FindField("Min")) = mn
//                        Case enumAnalysisType.AnalysisTypeSum
//                            pOutputFeat.Value( _
//                            pOutputFeat.Fields.FindField("Sum")) = s
//                        Case enumAnalysisType.AnalysisTypeCount
...
//                                Next I
//                                pOutputFeat.Value( _
//                                pOutputFeat.Fields.FindField("StDev")) = _
//                                Sqr(vari / (UBound(NeightborhoodData) - 1))
//                            End If
//                        Case enumAnalysisType.AnalysisTypeClusterAnalysis
//                            pOutputFeat.Value(pOutputFeat.Fields.FindField( _
...
//                        Case enumAnalysisType.AnalysisTypeMax
//                            PixelData(pOutputFeat.Value(OutputColFieldIndex) - 1, _
//                            pOutputFeat.Value(OutputRowFieldIndex) - 1) = mx
//                        Case enumAnalysisType.AnalysisTypeMin
//                            PixelData(pOutputFeat.Value(OutputColFieldIndex) - 1, _
//                            pOutputFeat.Value(OutputRowFieldIndex) - 1) = mn
//                        Case enumAnalysisType.AnalysisTypeSum
//                            PixelData(pOutputFeat.Value(OutputColFieldIndex) - 1, _
//                            pOutputFeat.Value(OutputRowFieldIndex) - 1) = s
//                        Case enumAnalysisType.AnalysisTypeCount
//                            PixelData(pOutputFeat.Value(OutputColFieldIndex) - 1, _
//                            pOutputFeat.Value(OutputRowFieldIndex) - 1) = C
//                        Case enumAnalysisType.AnalysisTypeMean
//                            PixelData(pOutputFeat.Value(OutputColFieldIndex) - 1, _
//                            pOutputFeat.Value(OutputRowFieldIndex) - 1) = s / C
//                        Case enumAnalysisType.AnalysisTypeStDev
//                            If UBound(NeightborhoodData) > 1 Then
//                                vari = 0
//                                For I = 1 To UBound(NeightborhoodData)
...
//                        Case enumAnalysisType.AnalysisTypeClusterAnalysis
//                            PixelData(pOutputFeat.Value(OutputColFieldIndex) - 1, _
//                            pOutputFeat.Value(OutputRowFieldIndex) - 1) = cc / C
//                    End Select
//            End Select
//        Else
//            Select Case OutputDatasetType
...
//                       Case enumAnalysisType.AnalysisTypeMax
//                            pOutputFeat.Value( _
//                            pOutputFeat.Fields.FindField("Max")) = -1
//                        Case enumAnalysisType.AnalysisTypeMin
//                            pOutputFeat.Value( _
//                            pOutputFeat.Fields.FindField("Min")) = -1
//                        Case enumAnalysisType.AnalysisTypeSum
//                            pOutputFeat.Value( _
//                            pOutputFeat.Fields.FindField("Sum")) = -1
//                        Case enumAnalysisType.AnalysisTypeCount
...
//                        Case enumAnalysisType.AnalysisTypeStDev
//                            pOutputFeat.Value( _
//                            pOutputFeat.Fields.FindField("StDev")) = -1
//                        Case enumAnalysisType.AnalysisTypeClusterAnalysis
//                            pOutputFeat.Value(pOutputFeat.Fields.FindField( _
//                            "ClusterPer")) = -1
//                    End Select
...
//Private Function IsWithinCluster(Analysis As AnalysisParams, v As Variant) As Boolean
//Dim I As Long

//    IsWithinCluster = False
//    For I = 1 To UBound(Analysis.CP)
//        Select Case Analysis.CP(I).Type
//            Case cnstClusterUniqueValue
//                If CStr(v) = Analysis.CP(I).Equale Then
//                    IsWithinCluster = True
//                    Exit For
//                End If
//            Case cnstClusterValueRange

//                If (CStr(v) >= Analysis.CP(I).From) And _
//                (CStr(v) <= Analysis.CP(I).To) Then
//                    IsWithinCluster = True
//                    Exit For
//                End If
//        End Select
...
//    pFieldEdit.Type = FieldType
//    pFieldEdit.IsNullable = False
//    pFieldEdit.DefaultValue = -1
//    If Not IsMissing(FieldLength) Then
//        pFieldEdit.Length = FieldLength
//    End If

//    If FieldType = esriFieldTypeGeometry Then
//        If Not IsMissing(GeometryType) Then
//            Dim pGeomDef As IGeometryDef
//            Dim pGeomDefEdit As IGeometryDefEdit
//            Set pGeomDef = New GeometryDef
//            Set pGeomDefEdit = pGeomDef
//            pGeomDefEdit.GeometryType = GeometryType
//            If Not IsMissing(SpatialReference) Then
//                Set pGeomDefEdit.SpatialReference = SpatialReference
//            End If
...
//Public Function ShowOpenFileDialog(pGxDialog As IGxDialog, _
//Path As String, Name As String, GenericName As String, DatasetType As String) As Boolean
//    Dim pEnumGX As IEnumGxObject
//    Dim pEnumGxObj As IGxObject

//    ShowOpenFileDialog = False

//    Path = "": Name = "": GenericName = ""
//    pGxDialog.Title = "Save File as:"

//    If Not pGxDialog.DoModalOpen(Application.hWnd, pEnumGX) Then Exit Function
//    DatasetType = pGxDialog.ObjectFilter.Name

//    pEnumGX.Reset
//    Set pEnumGxObj = pEnumGX.Next

//    GenericName = _
//    left(pEnumGxObj.Name, InStr(1, pEnumGxObj.Name, ".") - 1)
...
//Public Function CreateShapefile(Path As String, Name As String, _
//Fields As IFields) As IFeatureClass
//    Dim pFWS As IFeatureWorkspace
//    Dim pWorkspaceFactory As IWorkspaceFactory
//    Set pWorkspaceFactory = New ShapefileWorkspaceFactory
//    'Set pWorkspaceFactory = New RasterWorkspaceFactory
//    Set pFWS = pWorkspaceFactory.OpenFromFile(Path, 0)
//    Set CreateShapefile = pFWS.CreateFeatureClass(Name, Fields, Nothing, _
//                        Nothing, esriFTSimple, "Shape", "")
//End Function

//Public Function OpenShapefile(Path As String, Name As String) As IFeatureClass
//    Dim pFWS As IFeatureWorkspace
//    Dim pWorkspaceFactory As IWorkspaceFactory
//    Set pWorkspaceFactory = New ShapefileWorkspaceFactory
//    Set pFWS = pWorkspaceFactory.OpenFromFile(Path, Application.hWnd)
//    Set OpenShapefile = pFWS.OpenFeatureClass(Name)
//End Function
...
//Public Function CreateSpatialIndex(FeatClass As IFeatureClass) As Boolean
...
//    Dim h As Long
//    h = FeatClass.FindField("Shape")

//    Set pField = FeatClass.Fields.Field(h)
...
//Public Function CreateIndex(FeatClass As IFeatureClass, _
//ParamArray FieldsNames() As Variant) As Boolean
...
//    Set pFields = New Fields
//    Set pFieldsEdit = pFields
//    pFieldsEdit.FieldCount = UBound(FieldsNames) + 1

//    Dim h As Long
//    Dim I As Long
//    Dim IndexName As String
//    For I = 0 To UBound(FieldsNames)
//        h = FeatClass.FindField(FieldsNames(I))
//        Set pField = FeatClass.Fields.Field(h)
//        Set pFieldsEdit.Field(I) = pField
//        Set pField = Nothing
//        IndexName = IndexName & _
//        IIf(I = 0, FieldsNames(I), "," & FieldsNames(I))
//    Next I
...
//Public Function GetCursor(pTable As ITable, _
...
//    End If
//    Set GetCursor = pTable.Update(pQFilt, False)
//End Function
...
//Public Function GetReadOnlySortedCursor(pTable As ITable, _
//WhereClause As String, SortFields() As SortFieldsParams, _
//Optional RowCount As Variant) As icursor
...
//        .Ascending(SortFields(I).FieldName) = _
//        SortFields(I).Ascending
//        .CaseSensitive(SortFields(I).FieldName) = _
//        SortFields(I).CaseSensitive
//        For I = I + 1 To UBound(SortFields)
//            FieldsString = FieldsString & ", " & SortFields(I).FieldName
//            .Ascending(SortFields(I).FieldName) = _
//            SortFields(I).Ascending
//            .CaseSensitive(SortFields(I).FieldName) = _
//            SortFields(I).CaseSensitive
//        Next I
//        .Fields = FieldsString
//        Set .QueryFilter = pQFilt
//        Set .Table = pTable
//        .Sort Nothing
//        If Not IsMissing(RowCount) Then
//            RowCount = .Table.RowCount(pQFilt)
//        End If
//        Set GetReadOnlySortedCursor = .Rows
//    End With
//End Function

//Public Function GetReadOnlyFeatureCursor(pFeatClass As IFeatureClass, _
//WhereClause As String, Optional FeatureCount As Variant) As IFeatureCursor
//    Dim pQFilt As IQueryFilter
//    Set pQFilt = New QueryFilter
//    pQFilt.WhereClause = WhereClause
//    If Not IsMissing(FeatureCount) Then
//        FeatureCount = pFeatClass.FeatureCount(pQFilt)
//    End If
//    Set GetReadOnlyFeatureCursor = pFeatClass.Search(pQFilt, False)
//End Function

//Public Function GetUpdatableFeatureCursor(pFeatClass As IFeatureClass, _
//WhereClause As String, Optional SubFields As String = "*") As IFeatureCursor
//    Dim pQFilt As IQueryFilter
//    Set pQFilt = New QueryFilter
//    pQFilt.WhereClause = WhereClause
//    pQFilt.SubFields = SubFields
//    Set GetUpdatableFeatureCursor = pFeatClass.Update(pQFilt, False)
//End Function

//Public Function GetFeatureCentroid(Feature As IFeature) As IPoint
//    Dim m_Points As IPointCollection
//    Set m_Points = Feature.Shape
//    Dim pt As Integer
...
//            GoTo InvalidGeometryCalculationType
//            Exit Function
//    End Select
...
//        If pt = m_NumPoints - 1 Then
//            second_factor = _
//                m_Points.Point(pt).x * m_Points.Point(0).y - _
//                m_Points.Point(0).x * m_Points.Point(pt).y
//            x = x + (m_Points.Point(pt).x + m_Points.Point(0).x) * _
//                second_factor
//            y = y + (m_Points.Point(pt).y + m_Points.Point(0).y) * _
//                second_factor
//        Else
//            second_factor = _
//                m_Points.Point(pt).x * m_Points.Point(pt + 1).y - _
//                m_Points.Point(pt + 1).x * m_Points.Point(pt).y
//            x = x + (m_Points.Point(pt).x + m_Points.Point(pt + 1).x) * _
//                second_factor
//            y = y + (m_Points.Point(pt).y + m_Points.Point(pt + 1).y) * _
//                second_factor
//        End If
//    Next pt
...
//    pPoint.x = x
//    pPoint.y = y
//    Set GetFeatureCentroid = pPoint
//Exit Function

//InvalidGeometryCalculationType:
//  MsgBox "Invalid geometry calculation type." & vbCrLf & _
//    "Feature geometry does not support this type of calculation.", _
//     vbExclamation, "Can not proceed"
//Exit Function

//End Function

//Private Function SignedPolygonArea(Feature As IFeature) As Double
//Dim pt As Integer
//Dim area As Double
...
//        If pt = m_NumPoints - 1 Then
//            area = area + _
//                (m_Points.Point(0).x - m_Points.Point(pt).x) * _
//                (m_Points.Point(0).y + m_Points.Point(pt).y) / 2
//        Else
//            area = area + _
//                (m_Points.Point(pt + 1).x - m_Points.Point(pt).x) * _
//                (m_Points.Point(pt + 1).y + m_Points.Point(pt).y) / 2
//        End If
//    Next pt
...
//Public Function PolygonArea(Feature As IFeature) As Single
//    ' Return the absolute value of the signed area.
//    PolygonArea = Abs(SignedPolygonArea(Feature))
//End Function

//Public Function CustomToolsAddField(FeatureClass As IFeatureClass, FieldName As String, _
//FieldType As esriFieldType, Optional GeometryType As esriGeometryType, _
//Optional SpatialReference As ISpatialReference) As Boolean
//    CustomToolsAddField = False
//    If FieldType = esriFieldTypeGeometry Then
//        FeatureClass.AddField CreateField(FieldName, _
//        FieldType, , GeometryType, SpatialReference)
//    Else
//        FeatureClass.AddField CreateField(FieldName, _
//         FieldType)
//    End If
//    CustomToolsAddField = True
//End Function

//Public Function CustomToolsRemoveField(FeatureClass As IFeatureClass, _
//FieldName As String) As Boolean
//    CustomToolsRemoveField = False
//    FeatureClass.DeleteField _
//    FeatureClass.Fields.Field(FeatureClass.FindField(FieldName))
//    CustomToolsRemoveField = True
//End Function

//Public Function CustomToolsCalcGeometry(FeatureClass As IFeatureClass, _
//FieldName As String, CalculationType As enumGeometryCalculationType) As Boolean
//    CustomToolsCalcGeometry = False

//    Select Case CalculationType
//        Case GeometryClaculationTypeMinX, _
//        GeometryClaculationTypeMinY, _
//        GeometryClaculationTypeMaxX, _
//        GeometryClaculationTypeMaxY
//            If FeatureClass.ShapeType = esriGeometryPoint Then
//                GoTo InvalidGeometryCalculationType
//            End If
//        Case GeometryClaculationTypeArea, _
//        GeometryClaculationTypePerimeter, _
//        GeometryClaculationTypeCentroidX, _
//        GeometryClaculationTypeCentroidY
//            If FeatureClass.ShapeType <> esriGeometryPolygon Then
//                GoTo InvalidGeometryCalculationType
//            End If
//        Case GeometryClaculationTypePointX, _
//        GeometryClaculationTypePointY
//            If FeatureClass.ShapeType <> esriGeometryPoint Then
//                GoTo InvalidGeometryCalculationType
//            End If
//    End Select
...
//    FieldIndex = FeatureClass.FindField(FieldName)
//    If FieldIndex = -1 Then GoTo FieldIndexNotFound
//    Dim pFeat As IFeature
//    Dim pFeatCur As IFeatureCursor
//    Set pFeatCur = GetUpdatableFeatureCursor(FeatureClass, "1=1")
...
//        If FetchUserInterrupts = UserInterruptTypeStop Then Exit Function
//        Select Case CalculationType
//            Case GeometryClaculationTypeMinX
//                pFeat.Value(FieldIndex) = pFeat.Shape.Envelope.XMin
//            Case GeometryClaculationTypeMinY
//                pFeat.Value(FieldIndex) = pFeat.Shape.Envelope.YMin
//            Case GeometryClaculationTypeMaxX
//                pFeat.Value(FieldIndex) = pFeat.Shape.Envelope.XMax
//            Case GeometryClaculationTypeMaxY
//                pFeat.Value(FieldIndex) = pFeat.Shape.Envelope.YMax
//            Case GeometryClaculationTypeArea
//                pFeat.Value(FieldIndex) = pFeat.Shape.Envelope.YMax
//            Case GeometryClaculationTypePerimeter
//            Case GeometryClaculationTypeCentroidX
//                pFeat.Value(FieldIndex) = GetFeatureCentroid(pFeat).x
//            Case GeometryClaculationTypeCentroidY
//                pFeat.Value(FieldIndex) = GetFeatureCentroid(pFeat).y
//            Case GeometryClaculationTypePointX
//                Set pPoint = pFeat.Shape
//                pFeat.Value(FieldIndex) = pPoint.x
//                Set pPoint = Nothing
//            Case GeometryClaculationTypePointY
//                Set pPoint = pFeat.Shape
//                pFeat.Value(FieldIndex) = pPoint.y
//                Set pPoint = Nothing
//        End Select
...
//FieldIndexNotFound:
//    MsgBox "Field" & FieldName & "NotFound."

//    CustomToolsCalcGeometry = True
//End Function
...
//Public Sub TestCreateFileRaster()
//    Dim MxDoc As IMxDocument
//    Dim pSpatialRef As ISpatialReference
//    Set MxDoc = ThisDocument
//    Set pSpatialRef = MxDoc.FocusMap.SpatialReference
//    CreateFileRaster "C:\ArcGIS Files\TestCode", "TestRaster5.img", _
//    "IMAGINE Image", 127, 300, 100, 80, 0.03, 1, pSpatialRef, -1, True

//End Sub


// Public Function CreateFileRaster(Path As String, Name As String, Format As String, _
// OriginX As Double, OriginY As Double, ColumnCount As Long, RowCount As Long, _
// CellSize As Double, Bands As Integer, PixelData() As Variant, _
// Optional SpatialReference As ISpatialReference, Optional NoDataValue As Variant, _
// Optional Permanent As Boolean = True) As IRaster
//    'open a raster workspace
//    Dim pWs As IRasterWorkspace2
//    Set pWs = OpenRasterWorkspace(Path)
...
//    Set pRasterDs = pWs.CreateRasterDataset(Name, Format, _
//     pOrigin, Width, Height, CellSize, CellSize, Bands, PT_FLOAT, SpatialReference, Permanent)
...
//Public Function OpenFeatureWorkspace(sPath As String) As IFeatureWorkspace
//  ' Create FeatureWorkspace
//    Dim pWsFact As IWorkspaceFactory
//    Set pWsFact = New ShapefileWorkspaceFactory

//    If pWsFact.IsWorkspace(sPath) Then
//        Set OpenFeatureWorkspace = pWsFact.OpenFromFile(sPath, 0)
//    End If
